...
def schedule(galaxy, sum_to, max_digit):
    ...
    def schedule_helper(galaxy, sum_sofar, index):
        if galaxy == '' and sum_sofar == 0:
            return [galaxy]
        elif galaxy == '' and sum_sofar > 0 or sum_sofar < 0:
            return []
        elif galaxy[0] != '?':
            return [galaxy[0] + way for way in schedule_helper(galaxy[1:], sum_sofar - int(galaxy[0]), index)]
        ans = []
        for x in range(0, max_digit + 1):
            modified_galaxy = [str(x) + way for way in schedule_helper(galaxy[1:], sum_sofar - x, index)]
            ans.extend(modified_galaxy)
        return ans

    return schedule_helper(galaxy, sum_to, 0)
