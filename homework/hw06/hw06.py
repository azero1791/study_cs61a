...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            back_fund = self.balance - self.price
            self.balance = 0
            if back_fund == 0:
                return 'Here is your {0}.'.format(self.type)
            else:
                return 'Here is your {0} and ${1} change.'.format(self.type, back_fund)
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            ...
            bs_min = min([b.label for b in t.branches])
            return min(t.label, bs_min)

    def bst_max(t):
        if t.is_leaf():
            return t.label
        else:
            bs_max = max([b.label for b in t.branches])
            return max(t.label, bs_max)
    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 1:
            ...
            left = t.branches[0]
            right = t.branches[1]
            if not is_bst(left) or bst_max(left) > t.label:
                return False
            if not is_bst(right) or bst_min(right) <= t.label:
                return False
            return True
...
def store_digits(n):
    ...
    def helper(n, link):
        if n == 0:
            ...
            return helper(n // 10, Link(n % 10, link))
...
def remove_all(link , value):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(link, value, pre_link):
        if link == Link.empty:
            return 
        elif link.first == value:
            pre_link.rest = link.rest
            helper(pre_link.rest, value, pre_link)
        else:
            helper(link.rest, value, link)
    helper(link.rest, value, link)
