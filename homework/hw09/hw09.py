...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(n, odd_term, even_term, i, old, ans):
        if i > n:
            return ans
        elif old:
            return helper(n, odd_term, even_term, i + 1, False, ans + odd_term(i))
        else:
            return helper(n, odd_term, even_term, i + 1, True, ans + even_term(i))

    return helper(n, odd_term, even_term, 1, True, 0)
...
def mutate_reverse(link):
    ...
    while cur_link is not Link.empty:
        values.append(cur_link.first)
        cur_link = cur_link.rest
    while link is not Link.empty:
        link.first = values.pop()
        link = link.rest
