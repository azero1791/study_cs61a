...
def g_iter(n):
    ...
    while k <= n:
        first, second, third = second, third, 3 * first + 2 * second + 1 * third
        k += 1
    return third
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        if last > all_but_last % 10:
            return missing_digits(n // 10) + last - all_but_last % 10 - 1
        else:
...
def count_change(total):
    ...
    def helper(n, min_size):
        if n < 0:
            ...
            return helper(n, min_size * 2) + helper(n - min_size, min_size)
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda n : (lambda f, n: 1 if n == 1 else mul(n, f(f, n - 1)))((lambda func, n: 1 if n == 1 else mul(n, func(func, n - 1))), n)