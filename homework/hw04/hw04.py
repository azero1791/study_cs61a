...
def balanced(m):
    ...
    if length(left_arm) * total_weight(end(left_arm)) != length(right_arm) * total_weight(end(right_arm)):
        return False
    if not is_mobile(end(left_arm)):
        return True
    if not is_mobile(end(right_arm)):
        return True
    if not (is_mobile(end(left_arm)) and balanced(end(left_arm))):
        return False
    if not (is_mobile(end(right_arm)) and balanced(end(right_arm))):
        return False
    return True
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        left_arm, right_arm = left(m), right(m)
        branches = [totals_tree(end(left_arm)), totals_tree(end(right_arm))]
...
def has_path(t, phrase):
    ...
    if is_leaf(t):
        ...
        for b in branches(t):
            if has_path(b, phrase[1:]) and label(t) == phrase[0]:
                return True
        return False
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) != 0 and lower_bound(y) != 0 and upper_bound(y) * lower_bound(y) > 0
...
def quadratic(x, a, b, c):
    ...
    l_f, u_f = a*l_x**2 + b*l_x + c, a*u_x**2 + b*u_x + c
    if extreme_point < lower_bound(x) or extreme_point > upper_bound(x) or a == 0:
        return interval(min(l_f, u_f), max(l_f, u_f))
    elif a > 0:
        return interval(a*extreme_point**2 + b*extreme_point + c, max(l_f, u_f))
    else:
        return interval(min(l_f, u_f), a*extreme_point**2 + b*extreme_point + c)
