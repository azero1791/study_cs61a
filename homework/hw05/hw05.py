...
def make_withdraw(balance, password):
    ...
    count = 0
    error_lst = []
    def withdraw(amount, actual_p):
        nonlocal count,balance,error_lst
        if count >= 3:
            return f'Too many incorrect attempts. Attempts: {error_lst}'
        elif actual_p == password:
            if balance - amount < 0:
                return 'Insufficient funds'
            else:
                balance -= amount
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_pass_lst = [old_pass] 
    try_pass_end = withdraw(0, old_pass)
    if type(try_pass_end) == str:
        return try_pass_end
    new_pass_lst.append(new_pass)   
    def new_withdraw(amount, password):
        if password in new_pass_lst:
            withdraw_end = withdraw(amount, new_pass_lst[0])
        else:
            withdraw_end = withdraw(amount, password)
        return withdraw_end 
    return new_withdraw
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def get_number(remainder):
        for natural in naturals():
            if natural % m == remainder:
                yield natural
    for remainder in range(m):
        yield get_number(remainder)
