...
def subseqs(s):
    ...
    if s == []:
        ...
        nested_list = subseqs(s[1:])
        return insert_into_all(s[0], nested_list) + nested_list
...
def trade(first, second):
    ...
    while first[m:] != [] and second[n:] != [] and not equal_prefix():
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    total, gotten_score = 0, 0
    def table(key, actual):
        nonlocal total, gotten_score
        total += class_assignments[key]
        gotten_score += actual
        return gotten_score / total
    return table
...
def make_advanced_counter_maker():
    ...
    def g_counter():
        sub_g_count = 0
        def sub_g_counter(input):
            nonlocal g_count, sub_g_count
            "*** YOUR CODE HERE ***"
            if input == 'count':
                sub_g_count += 1
                return sub_g_count
