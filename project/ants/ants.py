...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        bee = None
        distance = 0
        while cur_place is not None and distance < self.min_range:
            distance += 1
            cur_place = cur_place.entrance

        while cur_place is not None and cur_place is not beehive and distance >= self.min_range and distance <= self.max_range:
            bee = rANTdom_else_none(cur_place.bees)
            if bee == None:
                cur_place = cur_place.entrance
            else:
                break
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.exist_queen:
            self.actual = True
            QueenAnt.exist_queen = True
        else:
            self.actual = False
        self.buffed_ants = []
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.actual:
            ...
            def make_buff(cur_place):
                while cur_place is not None:
                    if cur_place.ant is not None:
                        if isinstance(cur_place.ant, ContainerAnt) and \
                            cur_place.ant.contained_ant is not None and cur_place.ant.contained_ant not in self.buffed_ants:
                            cur_place.ant.contained_ant.damage *= 2
                            self.buffed_ants.append(cur_place.ant.contained_ant)
                        elif cur_place.ant not in self.buffed_ants:
                            cur_place.ant.damage *= 2
                            self.buffed_ants.append(cur_place.ant)
                    cur_place = cur_place.exit

            ThrowerAnt.action(self, gamestate)
            make_buff(self.place.exit)
