...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        diff_lst = [diff_function(user_word, valid_word, limit) for valid_word in valid_words]
        min_word, min_diff = min(zip(valid_words, diff_lst), key = lambda pair:pair[1])
        if min_diff > limit:
            return user_word
        else:
            return min_word
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    end = [[] for _ in range(len(players))]
    for word_i, word in zip(words, all_words(game)):
        word_times = [player_time[word_i] for player_time in all_times(game)]
        fastest_player = min(players, key = lambda ith: word_times[ith])
        end[fastest_player].append(word)
    return end 
    # END PROBLEM 10
...
def key_distance_diff(start, goal, limit):
    ...
    if limit < 0:
        ...
        delete_diff = 1 + key_distance_diff(start[1:], goal, limit - 1)
        
        dis = key_distance[(start[0], goal[0])]
        key_diff = dis + key_distance_diff(start[1:], goal[1:], limit - 1)
        return min(add_diff, delete_diff, key_diff)
    # END PROBLEM EC1
